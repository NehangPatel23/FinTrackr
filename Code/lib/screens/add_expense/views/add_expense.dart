...
import 'dart:math'; // Import to use `ceil()` for rounding up
...
class _AddExpenseState extends State<AddExpense> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['ParsedResults'] != null &&
          ...
          String extractedText = jsonResponse['ParsedResults'][0]['ParsedText'].trim();
  ...
  void extractAmountFromText(String text) {
    ...
    if (text.contains('total') || text.contains('t 01')) {
      ...
      RegExp amountRegex = RegExp(r'[\$]?\s*(\d{1,3}(?:,\d{3})*(?:\.\d{2})?)', caseSensitive: false); // Match numbers like 46.79, 1,000.00
      Match? match = amountRegex.firstMatch(remainingText);

      if (match != null) {
        String extractedAmount = match.group(1)!; // Extracted amount as a string

        // Convert to a double first
        double amount = double.tryParse(extractedAmount.replaceAll(',', '').replaceAll('\$', '')) ?? 0.0;

        // Round up to the nearest integer if it's a float
        if (amount != 0.0 && amount != amount.toInt()) {
          amount = (amount).ceil().toDouble(); // Round up and convert to double temporarily
        }
        ...
        setState(() {
          expenseController.text = roundedAmount.toString(); // Set the extracted and rounded amount as int
        });
